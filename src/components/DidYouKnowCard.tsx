...
/** Full-width banner, not a grid cell — squeezing this into one slot of
    ...
    instead of being locked to a box shaped for a photo. The photo (the
    widget's own consistent look, site_settings.facts_widget_bg_image —
    not a per-fact image) renders at full brightness with a navy tint
    layered on top at 75% opacity, not faded into the background — a
    fully dark photo faded to near-nothing read as invisible; tinting a
    full-strength photo keeps it recognisable while navy still visually
    dominates. */
export default function DidYouKnowCard({ fact, backgroundImage }: { fact: Fact; backgroundImage?: string }) {
  return (
    <Link
      href={`/facts/${fact.slug}`}
      className="group relative mb-5 block overflow-hidden bg-navy p-6 sm:mb-6 sm:p-8"
    >
      {backgroundImage && (
        <>
          <Image

      ...
      <section className="border-b border-navy/10 py-8 dark:border-white/10">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input type="number" min={0} step={1000} className={inputCls()} value={contractValue} onChange={(e) => setContractValue(Number(e.target.value) || 0)} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-navy/60 dark:text-white/50">Retention rate per claim (%)</label>
            <input type="number" min={0} max={20} step={0.5} className={inputCls()} value={retentionPct} onChange={(e) => setRetentionPct(Number(e.target.value) || 0)} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-navy/60 dark:text-white/50">Retention cap (% of contract value)</label>
            <input type="number" min={0} max={20} step={0.5} className={inputCls()} value={retentionCapPct} onChange={(e) => setRetentionCapPct(Number(e.target.value) || 0)} />
          ...
          <div>
            ...
            <input type="number" min={0} step={500} className={inputCls()} value={retentionHeldToDate} onChange={(e) => setRetentionHeldToDate(Number(e.target.value) || 0)} />
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-navy/10 py-8 dark:border-white/10">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input type="number" min={0} step={500} className={`${inputCls()} text-lg font-semibold`} value={claimValue} onChange={(e) => setClaimValue(Number(e.target.value) || 0)} />
          </div>
          <div>

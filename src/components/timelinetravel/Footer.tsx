...
import { getAllServices } from "@/lib/timelinetravel/services";
import { getFeaturedDestinations } from "@/lib/timelinetravel/destinations";
...
export default async function Footer() {
  ...
  const address = settings.contact_address || timelineTravelSiteConfig.address;
    ...
    <footer className="bg-[#0F3D3E] text-white/80">
      <div className="container-page grid gap-10 py-14 sm:grid-cols-2 lg:grid-cols-5">
        <div className="sm:col-span-2 lg:col-span-2">
          <Image
            src={timelineTravelSiteConfig.logo}
            alt={timelineTravelSiteConfig.name}
            width={160}
            height={48}
            className="h-10 w-auto brightness-0 invert"
          />
          <p className="mt-4 max-w-sm text-sm text-white/60">{timelineTravelSiteConfig.shortDescription}</p>
        </div>

        <FooterColumn
          title="Quick Links"
          items={timelineTravelSiteConfig.nav.map((n) => ({ label: n.label, href: n.href }))}
        />

...
// Kept as a standalone module (not layered onto the existing Paystack
// checkout code in src/lib/orders.ts) since the two providers work
// completely differently: Paystack is a REST "initialize + verify" API,
// PayFast is a signed form POST + server-to-server ITN validation.
// Andrew's own instruction was PayFast specifically, not "whatever the

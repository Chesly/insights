...
import { buildPaymentFields, PAYFAST_PROCESS_URL } from "@/lib/timelinetravel/payfast";
...
// done by /api/timelinetravel/bookings) and from verification (the ITN
// webhook below). Takes an existing pending booking and returns the
// PayFast form fields for the client to auto-submit; never itself marks
// anything as paid.
export async function POST(req: NextRequest) {
  ...
  const siteUrl = timelineTravelSiteConfig.url;
  ...
  const fields = buildPaymentFields({
    ...
    notifyUrl: `${siteUrl}/api/timelinetravel/webhooks/payfast`,
  });
